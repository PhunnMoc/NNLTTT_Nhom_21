...
public class EditCourseDialog extends JDialog {
    ...
    public EditCourseDialog(Frame owner, Course course, Runnable onSaved) {
        super(owner, "Sửa khóa học", true);
        this.course = course;
        buildUI(onSaved);
        loadCourse();
        pack();
    ...
    private void buildUI(Runnable onSaved) {
        ...
        btnSave.addActionListener(e -> {
            try {
                ...
                course.setLevel(lv != null ? lv.name() : null);
                course.setDuration(((Number) spinDuration.getValue()).intValue());
                String feeStr = txtFee.getText().trim();
                course.setFee(feeStr.isEmpty() ? null : new java.math.BigDecimal(feeStr));
                course.setStatus((String) comboStatus.getSelectedItem());
    ...
    private void loadCourse() {
        ...
        txtDescription.setText(course.getDescription() != null ? course.getDescription() : "");
        if (course.getLevel() != null) {
            try {
                comboLevel.setSelectedItem(Level.valueOf(course.getLevel()));
            } catch (Exception ignored) {}
        }
        spinDuration.setValue(course.getDuration() != null ? course.getDuration() : 1);
        txtFee.setText(course.getFee() != null ? course.getFee().toPlainString() : "");
        comboStatus.setSelectedItem(course.getStatus() != null ? course.getStatus() : "ACTIVE");
    }
